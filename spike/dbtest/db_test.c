...
#define		DYNAMODB_LOCAL_IP      "192.168.0.100"
...
#define     AWS_TARGET             "DynamoDB_20120810.ListTables"
...
//#define     DATE                   "20160824"
 ...
 int main()
 {
	...
	//	memcpy(&serv_addr.sin_addr.s_addr,ip_addr,server->h_length);
		uint8_t debug_i = 0;
		
		for (debug_i = 0; debug_i < server->h_length; debug_i++)
		{
			printf("ip_addr=");
			printf("0x%02x ",server->h_addr[debug_i]);
			printf("\n");
		}
		printf("ip_length=%d\n",server->h_length);
